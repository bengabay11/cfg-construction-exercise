...
def get_leader_indexes(instructions: List) -> List[int]:
    if not instructions:
        return []
    leader_indexes = {0}
    for index, instruction in enumerate(instructions):
        if isinstance(instruction, Jump):
            if instruction.condition and index < len(instructions) - 1:
                leader_indexes.add(index + 1)
            leader_indexes.add(instruction.target)
    return sorted(leader_indexes)


def get_basic_blocks(instructions: List, leader_indexes: List) -> List[List]:
...
def create_jump_edges(graph: nx.Graph, leader_indexes: List[int], current_basic_block_index: int, jump: Jump) -> None:
    if jump.target in leader_indexes:
        target_basic_block_index = leader_indexes.index(jump.target)
        if jump.condition and current_basic_block_index < len(graph.nodes) - 1:
...
def create_cfg_nodes(graph: nx.Graph, basic_blocks: List) -> None:
    for i, basic_block in enumerate(basic_blocks):
        graph.add_node(i, instructions=basic_block, level=len(basic_blocks)-1-i)


def create_cfg_edges(graph: nx.Graph, basic_blocks: List, leader_indexes: List[int]) -> None:
